...
description = "Use this skill to perform a strictly read-only audit of a Git repository, including local and remote branches, worktrees, suspicious committed files, repository hygiene risks, and GitHub issues when accessible. Produces a prioritized cleanup and handoff report without modifying any repository or GitHub state."
license = "MIT"
compatibility = "Model-agnostic Agent Skill. Requires a POSIX-like shell for the bundled script. Optional tools: git, gh, jq, find, grep, awk, sed, sort, comm, wc, du."
allowed_tools = ["Read", "Bash"]
+++

# Repo Cleanup Audit (Read-Only)

This skill guides an AI coding agent through a strictly read-only audit of a
software repository. The agent inspects branches, remotes, worktrees, history,
on-disk hygiene risks, and (when available) GitHub issues, then emits a
prioritized cleanup report and a handoff document so a human, another agent,
or a follow-up chat can decide what to clean up.
...
Do **not** activate this skill for active development tasks (writing code,
fixing bugs, modifying configuration). This skill produces *findings*, not
changes.

## Read-only command policy (CRITICAL)

This skill is **strictly read-only**. The agent must not run any command that
mutates the repository, working tree, Git history, configuration, remotes,
worktrees, GitHub state (issues, labels, milestones, PRs, releases), or any
file outside a clearly-marked temporary report directory.

### Forbidden commands (non-exhaustive)

The following commands and their variants are forbidden during this skill,
...
- An optional output directory for the report. If not provided, the agent
  ...
  user before writing files. This skill writes only to the report directory
  it announces.
...
Run `scripts/audit.sh` from the skill directory, passing the repository
path as the first argument and an output directory as the second. The script
is read-only and self-contained. Example:

```bash
bash <skill-dir>/scripts/audit.sh "$REPO_PATH" "$OUT_DIR"
```
...
- **CI-only checkouts.** If the repo lacks a `.git` directory but contains
  source, abort cleanly — this skill needs Git metadata.

## Expected deliverables

By the end of an activation, the agent must have produced:
...
- [ ] The repository is at the same commit, with the same staged/unstaged
      state, as before activation. Run `git status --porcelain=v2 --branch`
...
3. For low/medium items, draft a cleanup PR proposal — still as a separate
   task, not within this skill.

## Read-only contract

This skill produced no changes. Any cleanup must be performed by a separate,
explicitly authorized task with full human review.
```

...
> **Script paths.** All `scripts/...` paths in this document are relative to the install directory of this command's support tree. Under Claude Code that's `.claude/github-issue-delivery/`, so the agent should call `.claude/github-issue-delivery/scripts/init-artifacts.sh ...`. Other clients should substitute their install path.
